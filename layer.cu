...
Batchnorm::Batchnorm(std::string name,int num_feature, double momentum, double eps)
{
	name_ = name;
...
Blob<float> *Batchnorm::forward(Blob<float> *input)
{
	input_ = input;
	init();
	if (batch_size_ != input_->n())
	{
		// resource initialize
		...
		if (output_ == nullptr)
		{
			...
			grad_output_ = new Blob<float>(output_->shape());
		}
		else
		{
	...
	if (weights_ == nullptr)
	{
		weights_ = new Blob<float>(1, num_feature_, 1, 1);
		biases_ = new Blob<float>(1, num_feature_, 1, 1);
		running_mean_ = new Blob<float>(1, num_feature_, 1, 1);
		running_var_ = new Blob<float>(1, num_feature_, 1, 1);
	...
	//output_->allocate_on_gpu();

	// ADD CUDA KERNEL
	checkCudnnErrors(cudnnBatchNormalizationForwardTraining(cuda_->cudnn(), CUDNN_BATCHNORM_SPATIAL,
		&cuda_->one, &cuda_->zero, input_->tensor(), input_->cuda(), output_->tensor(), output_->cuda(), weights_->tensor(),
		weights_->cuda(), biases_->cuda(), momentum_, running_mean_->cuda(), running_var_->cuda(), eps_, running_mean_->cuda(), running_var_->cuda()));

	return output_;
}

Blob<float> *Batchnorm::backward(Blob<float> *grad_output)
{
	grad_output_ = grad_output;
	...
	//grad_input_->allocate_on_gpu();

	checkCudnnErrors(cudnnBatchNormalizationBackward(cuda_->cudnn(), CUDNN_BATCHNORM_SPATIAL, &cuda_->one, &cuda_->one, &cuda_->zero, &cuda_->one, input_->tensor(), input_->cuda(),
	grad_output_->tensor(), grad_output_->cuda(), grad_input_->tensor(), grad_input_->cuda(), weights_->tensor(), weights_->cuda(),
	weights_->cuda(), biases_->cuda(), eps_, running_mean_->cuda(), running_var_->cuda()));
